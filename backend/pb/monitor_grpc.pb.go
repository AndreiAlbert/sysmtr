...
const (
	MonitorService_PushStats_FullMethodName = "/monitor.MonitorService/PushStats"
)
...
type MonitorServiceClient interface {
	PushStats(ctx context.Context, opts ...grpc.CallOption) (grpc.ClientStreamingClient[SystemStats, Empty], error)
}
...
func (c *monitorServiceClient) PushStats(ctx context.Context, opts ...grpc.CallOption) (grpc.ClientStreamingClient[SystemStats, Empty], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &MonitorService_ServiceDesc.Streams[0], MonitorService_PushStats_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[SystemStats, Empty]{ClientStream: stream}
	return x, nil
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type MonitorService_PushStatsClient = grpc.ClientStreamingClient[SystemStats, Empty]
...
type MonitorServiceServer interface {
	PushStats(grpc.ClientStreamingServer[SystemStats, Empty]) error
...
func (UnimplementedMonitorServiceServer) PushStats(grpc.ClientStreamingServer[SystemStats, Empty]) error {
...
func _MonitorService_PushStats_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(MonitorServiceServer).PushStats(&grpc.GenericServerStream[SystemStats, Empty]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type MonitorService_PushStatsServer = grpc.ClientStreamingServer[SystemStats, Empty]
	...
	Streams: []grpc.StreamDesc{
		{
			...
			Handler:       _MonitorService_PushStats_Handler,
			ClientStreams: true,
		},
	},
